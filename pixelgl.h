...
#pragma endregion Macros
    ...
    // TODO: saving, and operation directly on img pixelbuffer rather than copy
    class Image
    {
    ...
    public:
        ...
        {
            img = png::image<png::rgb_pixel>(path);
            ...
            for (int ix = 0; ix < dimensions.x; ix++)
            {
                ...
                {
                    png::rgb_pixel px = img.get_pixel(ix, iy);
                    pixelBuffer[ix + (ix * dimensions.x)] = {px.red, px.green, px.blue};
                }
            }
        }

        Vector2<unsigned int> getDimensions() {
            return dimensions;
        }
    ...
    // void ReadPNG(const std::string &path, Vector2<unsigned int>* outDim, Pixel* outData)
    // {
    ...
    //     if (!fp)
    //         vthrowf("PixelGL::readPNG File %s could not be opened for reading", path.c_str());
    //     fread(header, 1, 8, fp);
    //     if (png_sig_cmp(header, 0, 8))
    //         vthrowf("PixelGL::readPNG File %s is not recognized as a PNG file", path.c_str());

    //     /* initialize stuff */
    //     png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);

    //     if (!png_ptr)
    //         vthrow("PixelGL::readPNG png_create_read_struct failed");

    //     info_ptr = png_create_info_struct(png_ptr);
    //     if (!info_ptr)
    //         vthrow("PixelGL::readPNG png_create_info_struct failed");

    //     if (setjmp(png_jmpbuf(png_ptr)))
    //         vthrow("PixelGL::readPNG Error during init_io");
    ...
    //     outDim->x = png_get_image_width(png_ptr, info_ptr);
    //     outDim->y = png_get_image_height(png_ptr, info_ptr);
    ...
    //     /* read file */
    //     if (setjmp(png_jmpbuf(png_ptr)))
    //         vthrow("PixelGL::readPNG Error during read_image");
    ...
    //     row_pointers = (png_bytep *)malloc(sizeof(png_bytep) * outDim->y);
